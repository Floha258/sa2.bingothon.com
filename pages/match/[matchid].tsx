...
import PlayerHeader from '../../components/TeamHeader.js';
...
export default function MatchView(props: MatchDataProps) {
    const { matchData, homeTeamData, awayTeamData } = props;
    const boardId = matchData.bingosyncBoardId;
    const { data, error } = useSWR(`/api/matchData/${boardId}`, fetcher);
    /*let eloSlug = '';
    if (matchData.winner == matchData.homeTeam && matchData.homePlayerEloGain) {
                ...
                <div className="flex mx-auto w-1/2">
                    <div className="mx-auto w-2/5">
                        <PlayerHeader
                            ...
                            countryCode={homeTeamData.countryP1}
                    ...
                    <div className="mx-auto w-2/5">
                        <PlayerHeader
                            ...
                            countryCode={awayTeamData.countryP1}
...
export const getStaticProps: GetStaticProps = async context => {
    ...
    const matchData = convertAirtableDataToMatchData(await base(process.env.AIRTABLE_COMPETITORS_TABLE_NAME).find(matchId));
    const homePlayerData = convertAirtableDataToPlayerData(
        await base(process.env.AIRTABLE_COMPETITORS_TABLE_NAME).find(matchData.homeTeamId)
    );
    const awayPlayerData = convertAirtableDataToPlayerData(
        await base(process.env.AIRTABLE_COMPETITORS_TABLE_NAME).find(matchData.awayTeamId)
    );
    return {
        props: {
            matchData,
